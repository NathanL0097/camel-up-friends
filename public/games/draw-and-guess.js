(() => {
  ...
  window.GameClientFactories["draw-and-guess"] = ({ socket, $, show, escapeHtml, getMyId, copyInvite }) => {
    ...
    let renderedCanvasRevision = null;
    ...
    $("rulesContent").innerHTML = `<div class="eyebrow">轻松上手 · 纯名词词库</div><h2>你画我猜 · 规则</h2><ol><li><strong>轮流作画：</strong>每位玩家会担任三次画家，游戏自动决定顺序。</li><li><strong>秘密选词：</strong>每批固定包含“直观、进阶、挑战”三种难度；都不满意可以不限次数换一批。</li><li><strong>题目不回收：</strong>一个词被选中开画后会进入永久废题库，以后房间不再抽到。</li><li><strong>纯名词题库：</strong>只出现能独立画出的物品、动植物、人物、角色、建筑和自然景观。</li><li><strong>限时75秒：</strong>画家只能用画板表达，不能直接写出答案。</li><li><strong>计分：</strong>猜得越早分数越高；本轮只要有任意一人猜中，画家一次性获得40分，不会随猜中人数重复加分。三人及以上无人猜中时画家扣60分，双人局不扣。</li><li><strong>欢乐评价：</strong>答案揭晓后，其他玩家可以给画作点赞或扔鸡蛋，不影响分数。</li><li><strong>提前结束：</strong>所有在线猜题玩家都猜中后，立即进入揭晓。</li><li><strong>最终彩蛋：</strong>所有人完成三次作画后，冠军可为最后一名抽取健康的真心话或大冒险。</li></ol>`;
    ...
    function render(room) {
      ...
      if (choosingMine) {
        $("wordChoice").innerHTML = `<small>选择你最想画的题目 · 每批三种难度</small><div>${game.wordChoices.map((choice) => `<button data-word="${escapeHtml(choice.word)}"><span>${escapeHtml(choice.category)} · ${wordDifficultyNames[choice.difficulty] || "聚会"}</span><strong>${escapeHtml(choice.word)}</strong></button>`).join("")}</div><button id="refreshWords" class="refresh-words">🔄 都不满意，换一批</button>`;
      ...
      if (game.phase === "reveal" && game.lastResult) {
        ...
        const noGuessText = result.penaltyWaived ? "无人猜中 · 双人局画家不扣分" : `无人猜中 · 画家扣 ${result.artistPenalty} 分`;
        reveal.innerHTML = `<small>本轮答案</small><strong>${escapeHtml(result.word)}</strong><p>${result.correctPlayers.length ? `${result.correctPlayers.length} 人猜中 · 画家 ${escapeHtml(result.artistName)} 获得 ${result.turnScores[result.artistId] || 0} 分` : noGuessText}</p><div class="correct-score-list">${result.correctPlayers.map((player) => `<span>${escapeHtml(player.playerName)} +${player.score}</span>`).join("")}</div>${reactionButtons}<i class="reaction-pop ${myReaction || ""}">${myReaction === "like" ? "👍" : myReaction === "egg" ? "🥚" : ""}</i>`;
      ...
      const structuralChange = renderedTurn !== turnKey || renderedCanvasRevision !== game.canvasRevision || renderedStrokeLengths.size !== currentLengths.size
        || [...renderedStrokeLengths].some(([id, length]) => !currentLengths.has(id) || currentLengths.get(id) < length);
      if (structuralChange) {
        renderedTurn = turnKey;
        renderedCanvasRevision = game.canvasRevision;
