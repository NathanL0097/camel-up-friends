(() => {
  ...
  window.GameClientFactories["market-opening"] = ({ socket, $, show, escapeHtml, getMyId, copyInvite }) => {
    ...
    function renderDraft(room) {
      ...
      draft.innerHTML = `<div class="draft-heading"><div><span class="draft-kicker">角色顺位已确定</span><h2>${myTurn ? "轮到你选择商业奇才" : `等待 ${escapeHtml(playerById[game.draftCurrentPlayerId]?.name || "玩家")} 选择`}</h2><p>点数从高到低依次选择，每个角色一局中只会出现一次。</p></div><div class="draft-order">${game.draftOrder.map((entry, index) => { const player = playerById[entry.playerId]; return `<div class="draft-seat ${entry.playerId === game.draftCurrentPlayerId ? "current" : ""}"><b>${index + 1}</b><span>${escapeHtml(player?.name || "玩家")}</span><em>点数 ${entry.roll}</em>${player?.character ? `<small>${escapeHtml(player.character.name.slice(0, 1))} ${escapeHtml(player.character.name)}</small>` : ""}</div>`; }).join("")}</div></div><div class="character-grid">${game.availableCharacters.map((character) => `<button class="character-card ${character.type}" data-character="${character.id}" ${myTurn ? "" : "disabled"}><span class="character-avatar">${escapeHtml(character.name.slice(0, 1))}</span><span class="character-type">${character.type === "passive" ? "开局被动" : "一次性技能"}</span><strong>${escapeHtml(character.name)}</strong><small>${escapeHtml(character.description)}</small><em>选择角色 →</em></button>`).join("")}</div>`;
    ...
    function renderSkill(room) {
      ...
      if (character.type === "passive") {
        panel.innerHTML = `<div class="skill-avatar">${escapeHtml(character.name.slice(0, 1))}</div><div><small>你的角色 · 被动生效</small><strong>${escapeHtml(character.name)}</strong><p>${escapeHtml(character.description)}</p></div><b>常驻</b>`;
        return;
      }
      if (character.skillUsed) {
        panel.innerHTML = `<div class="skill-avatar used">${escapeHtml(character.name.slice(0, 1))}</div><div><small>你的角色 · 技能已发动</small><strong>${escapeHtml(character.name)}</strong><p>${me.skillInfo ? `侦察结果：${escapeHtml(me.skillInfo)}` : escapeHtml(character.description)}</p></div><b class="used">已用</b>`;
        return;
      }
    ...
    function renderPlayers(room) {
      ...
      $("marketPlayers").innerHTML = players.map((player) => {
        ...
        return `<div class="market-player ${mine ? "mine" : ""} ${player.ready && !waiting ? "ready" : ""}"><div class="market-player-name"><span>${finished ? `<b>#${player.rank}</b>` : player.connected ? "●" : "○"} ${mine ? "你 · " : ""}${escapeHtml(player.name)}</span><em>${player.shares} 股</em></div>${player.character ? `<div class="player-character"><i>${escapeHtml(player.character.name.slice(0, 1))}</i><span>${escapeHtml(player.character.name)}</span><small>${player.character.type === "passive" ? "被动" : player.character.skillUsed ? "技能已用" : "技能可用"}</small></div>` : `<div class="player-character pending">等待选角</div>`}${mine || finished ? `<div class="private-wallet private-wallet-values"><span><small>股票资金</small><strong>${player.cash}</strong></span><span><small>预测金币</small><strong>${player.predictionCoins} 🪙</strong></span></div>` : `<div class="private-wallet"><span>${player.ready ? player.effectReady || room.game.round === room.game.totalRounds ? "决定已锁定" : "正在选择消息" : "思考中…"}</span><span>资金 🔒</span></div>`}${finished ? `<div class="final-player-wealth">最终财富 <strong>${player.finalWealth}</strong>${player.characterBonus ? `<small>含角色加成 +${player.characterBonus}</small>` : ""}</div>` : ""}</div>`;
      }).join("");
    }
