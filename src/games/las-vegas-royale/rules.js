...
const TILES = [
  ...
  { id: "E1", name: "禁止入场", icon: "⛔", rule: "激活时选择另一座赌场封锁，直到本板块下次被激活。每次发动还会推进奖励轨，并按所在位置领取对应奖金。" },
  { id: "E2", name: "淘汰出局", icon: "🥊", rule: "激活时，每名对手将一颗剩余骰放入等待区，每人最多被扣住2颗；激活者同时取回自己等待区中的全部骰子。" },
  { id: "F1", name: "堵住它", icon: "🧱", rule: "激活时选择一组灰骰，并放入任意未封锁赌场。灰骰作为中立票数参与平票与排名，会干扰所有玩家的奖金争夺。" },
  { id: "F2", name: "让分局", icon: "♿", rule: "激活时可移除场上一颗灰骰，换取板块上尚未领取的奖励：1枚筹码、$30K，或操纵一颗自己的已放置骰子。也可以放弃。" },
  { id: "G1", name: "黑箱", icon: "⬛", rule: "赛段结算时，本赌场第一名左边的玩家把2枚筹码、$40K、$60K、$80K、$100K分成两组；第一名只能选择其中一组并领取全部奖励。" },
  { id: "G2", name: "双倍下注", icon: "×2", rule: "激活时可把本赌场任意数量的己方骰移到副桌。赛段结算时副桌独立比较票数，第一名获得$60K，第二名获得$30K。" },
  { id: "H1", name: "妙骰", icon: "✨", rule: "可将刚放下的一颗骰移到本赌场对应的妙骰格；同点格已有骰时，原骰返回对应赌场。赛段结算按格子奖励：1至2点给筹码，3至6点给$30K至$60K。" },
  { id: "H2", name: "任我选", icon: "👑", rule: "激活时掷2颗黑骰，从出现的点数中选一个效果：获得筹码、$30K、激活另一豪华板块、操纵己方骰子，或占据价值$60K的金色奖励格。" }
];
const TILE_GUIDES = {
  A1: { trigger: "你的骰子进入这座赌场，立即触发。", action: "你秘密选1、2或3枚标记；左手边玩家只能猜一个数字。", result: "对方猜中：什么都没有。猜错：你按所握数量领取奖励。", example: "例如你握3枚，对方猜2枚，你获得$40K；若对方猜3枚，你得不到奖励。" },
  ...
  E1: { trigger: "你的骰子进入这座赌场，立即触发。", action: "选择另一座赌场放上禁止入场标记；被封锁赌场暂时不能接收骰子。", result: "旧封锁会被新封锁替换，同时本板块奖励轨前进并可能发放奖金。", example: "例如你封锁6号赌场，之后任何玩家本次掷出的6点都不能放入6号，直到本板再次触发。" },
  E2: { trigger: "你的骰子进入这座赌场，立即触发。", action: "每名对手若还有骰子，就把1颗放入等待区；每人最多被扣2颗。", result: "你同时取回自己等待区里的全部骰子，因此可以再次使用它们。", example: "例如两名对手各剩4颗骰，他们各被扣1颗；你之前被扣2颗，则这2颗全部回到手中。" },
  F1: { trigger: "你的骰子进入这座赌场，且板块还有灰骰组时触发。", action: "选择一组1、2或3颗灰骰，再选择一座未封锁赌场放入。", result: "灰骰作为中立玩家参与票数比较，能够制造平票并让玩家失去奖金。", example: "例如对手在4号有2票，你放入2颗灰骰形成2比2；结算时双方同票，一起被淘汰。" },
  F2: { trigger: "你的骰子进入这里，且场上仍有灰骰时触发。", action: "可从任意未封锁赌场移除1颗灰骰，再选择板块上一个尚未领取的奖励。", result: "立即获得1枚筹码、$30K，或把自己一颗已放置骰改到其他赌场；也可以放弃。", example: "例如移除4号赌场的一颗灰骰并选择$30K格，你立即拿钱，灰骰也不再参与结算。" },
  G1: { trigger: "不是放骰时触发，而是本赌场完成排名、出现第一名后触发。", action: "第一名左边的玩家，把6项奖励秘密分成两个都不为空的组。", result: "第一名只能二选一，翻开并取得所选组中的全部筹码和金钱。", example: "例如A组为$100K，B组为2筹码+$40K+$60K+$80K；第一名选一组，另一组作废。" },
  G2: { trigger: "你的骰子进入这座赌场后，立即决定是否分出骰子。", action: "把这里0颗、1颗或多颗自己的骰子移到旁边的副桌；Biggy仍算2票。", result: "本轮结算时副桌单独排名：第一名$60K，第二名$30K；主赌场奖金另算。", example: "例如你把2颗骰移到副桌，对手放1颗；副桌结算你第一拿$60K，不影响主桌排名。" },
  H1: { trigger: "你的骰子进入这里后，可选择刚放下的一颗骰子。", action: "把这颗骰移到与本赌场点数相同的妙骰奖励格；也可以不移动。", result: "赛段结算时按格子领奖：1、2点给筹码，3至6点分别给$30K至$60K。", example: "例如板块在5号赌场，把刚放的骰移到5点妙骰格，结算时该骰为你带来$50K。" },
  H2: { trigger: "你的骰子进入这座赌场，立即掷2颗黑骰。", action: "两颗黑骰出现哪些点数，你就从这些点数对应的效果中选择一个。", result: "1/2点给筹码，3点给$30K，4点触发另一板块，5点操纵己方骰，6点争夺$60K。", example: "例如黑骰为3和6，你只能在立即拿$30K，或占据价值$60K的金色奖励格之间选择。" }
};
TILES.forEach((tile) => { tile.guide = TILE_GUIDES[tile.id]; });

function defaults() {
  return { mode: "royale", tileCount: 3 };
}

function configure(room, playerId, payload = {}) {
  if (room.hostId !== playerId) throw new Error("只有房主可以修改赌城规则");
  if (room.game) throw new Error("游戏已经开始");
  const mode = payload.mode === "base" ? "base" : "royale";
  const tileCount = mode === "base" ? 0 : Math.max(1, Math.min(6, Number(payload.tileCount) || 3));
  room.settings = { mode, tileCount };
}
...
const animate = (game, type, detail = {}) => {
  game.eventSeq = (game.eventSeq || 0) + 1;
  game.animationEvents ||= [];
  game.animationEvents.push({ id: game.eventSeq, type, at: Date.now(), ...detail });
  if (game.animationEvents.length > 100) game.animationEvents.splice(0, game.animationEvents.length - 100);
};
...
function makeRoundMoney(deck) {
  ...
  // 官方规则：总额最高的一对放在6号赌场，依次降到1号赌场。
  return pairs.sort((a, b) => (a[0] + a[1]) - (b[0] + b[1]) || a[0] - b[0]);
}

function chooseTiles(count) {
  // 八块实体板每块只有 A-H 两面，同一轮不能同时出现同一块的正反面。
  return shuffle(["A", "B", "C", "D", "E", "F", "G", "H"])
    .slice(0, count)
    .map((letter) => {
      const side = Math.random() < .5 ? 1 : 2;
      return TILES.find((tile) => tile.id === `${letter}${side}`);
    });
}
...
function createGame(players, settings = defaults()) {
  ...
  const game = {
    ...
    settings: { ...defaults(), ...settings, tileCount: settings.mode === "base" ? 0 : Math.max(1, Math.min(6, Number(settings.tileCount) || 3)) },
    playerState: {}, roundMoney: [makeRoundMoney(deck), makeRoundMoney(deck), makeRoundMoney(deck)],
    bank: deck, casinos: [], currentRoll: null, pending: null, settlement: null, log: [], eventSeq: 0, animationEvents: []
  };
...
function roll(room, playerId) {
  ...
  animate(game, "dice-roll", { playerId, dice: game.currentRoll.map(({ face, big }) => ({ face, big })), reason: "行动掷骰" });
...
function pass(room, playerId) {
  ...
  if (hasLegalFace) { state.chips -= 1; animate(game, "chips", { playerId, amount: -1, reason: "支付筹码跳过" }); }
  game.currentRoll = null;
...
function reward(game, playerId, cash = 0, chips = 0, reason = "豪华板块奖励") {
  addCash(game, playerId, cash, reason);
  if (chips) {
    playerState(game, playerId).chips += chips;
    animate(game, "chips", { playerId, amount: chips, reason });
  }
}
  ...
  switch (pending.type) {
    ...
    case "luckyGuess": {
      ...
      if (guess !== pending.secretCount) {
        animate(game, "reveal", { playerId: pending.ownerId, reason: `握有${pending.secretCount}枚，猜测失败` });
        if (pending.secretCount === 1) reward(game, pending.ownerId, 0, 2, "幸运一拳");
        else reward(game, pending.ownerId, pending.secretCount === 2 ? 30 : 40, 0, "幸运一拳");
        log(game, "猜错了，幸运一拳的奖励由激活者获得。");
      } else { animate(game, "reveal", { playerId: pending.ownerId, reason: `握有${pending.secretCount}枚，猜中了` }); log(game, "猜中了！幸运一拳没有发出奖励。"); }
    ...
    case "myChoice": {
      ...
      if (option === 1) reward(game, playerId, 0, 1, "任我选");
      else if (option === 2) reward(game, playerId, 0, 2, "任我选");
    ...
    case "blackChoose": {
      ...
      animate(game, "reveal", { playerId, reason: `黑箱${pile === 0 ? "A" : "B"}组揭晓` });
      pending.piles[pile].forEach((i) => rewards[i] === "chip" ? reward(game, playerId, 0, 1, "黑箱奖励") : reward(game, playerId, rewards[i], 0, "黑箱奖励"));
...
function continueSettlement(room) {
  ...
  if (settlement.casinoIndex < 6) {
    ...
    if (casino.tile?.id === "C1" && casino.tile.state.ownerId) addCash(game, casino.tile.state.ownerId, 100, "五骰同堂");
    if (casino.tile?.id === "G2") {
      ...
      if (sideRanks[0]) addCash(game, sideRanks[0][0], 60, "双倍下注第一名");
      if (sideRanks[1]) addCash(game, sideRanks[1][0], 30, "双倍下注第二名");
    }
  ...
  if (!settlement.nextStarter) {
    for (let index = 5; index >= 0 && !settlement.nextStarter; index -= 1) {
      const winner = untiedRanking(game.casinos[index]).slice(0, 1)[0]?.[0];
      if (winner && !winner.startsWith("__")) settlement.nextStarter = winner;
    }
  }
  endRound(room);
}
...
function endRound(room) {
  const game = room.game;
  game.lastSettlement = { id: `round-${game.round}-${Date.now()}`, round: game.round, awards: game.settlement.awards };
...
function publicRoom(room, viewerId) {
  ...
  if (!game) return { code: room.code, hostId: room.hostId, players, settings: room.settings || defaults() };
  ...
  return {
    code: room.code, hostId: room.hostId, players, settings: game.settings,
    game: {
