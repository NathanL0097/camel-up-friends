...
test("预言家只判断主牌方向，猜中获得三金币", () => {
  const testRoom = room();
  draft(testRoom, "prophet", "operator");
  testRoom.game.openingEvent.impact = 0;
  testRoom.game.marketDeck[0] = -20;
  rules.useSkill(testRoom, "p1", { direction: "down" });
  submitRound(testRoom, { prediction: "none", wager: 0, trade: "hold", shares: 0 });
  assert.equal(testRoom.game.lastEvent.prophetResults[0].correct, true);
  assert.equal(testRoom.game.finances.p1.coins, 23);
});
...
test("最终财富按最后股价结算并加入长线投资家被动", () => {
  const finance = { cash: 300, shares: 15, coins: 24 };
  assert.equal(rules.finalWealth(finance, 70), 2550);
  assert.equal(rules.finalWealth(finance, 70, 75), 2625);
});

test("不能透支、做空或持有超过二十股", () => {
  const testRoom = room();
  draft(testRoom);
  assert.throws(() => rules.submitDecision(testRoom, "p1", { prediction: "none", wager: 0, trade: "buy", shares: 21 }), /最多持有/);
  assert.throws(() => rules.submitDecision(testRoom, "p1", { prediction: "none", wager: 0, trade: "sell", shares: 1 }), /持股数量不足/);
});
